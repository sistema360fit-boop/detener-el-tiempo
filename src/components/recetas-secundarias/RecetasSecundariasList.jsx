...
export default function RecetasSecundariasList({ recetas, detalles, onEdit, onDelete, isLoading }) {
  if (isLoading) {
    return (
    ...
    return (
      <Card className="shadow-lg">
        <CardContent className="p-12 text-center">
          ...
          <p className="text-gray-500">No hay recetas secundarias creadas</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {recetas.map(receta => {
        const elementosReceta = detalles.filter(d => d.receta_secundaria_id === receta.id);
            ...
            <CardContent className="p-6 space-y-4">
              ...
              <div className="border-t pt-4">
                <p className="text-sm font-semibold text-gray-700 mb-2">
                  Elementos ({elementosReceta.length})
                </p>
                <div className="space-y-1">
                  {elementosReceta.slice(0, 3).map((det, idx) => (
                    <div key={idx} className="flex items-center gap-2 text-sm">
                      <Badge className={det.tipo_elemento === "receta_primaria" ? "bg-amber-100 text-amber-800 text-xs" : "bg-green-100 text-green-800 text-xs"}>
                        {det.tipo_elemento === "receta_primaria" ? "R" : "I"}
                      </Badge>
                      <p className="text-gray-600">
                        {det.elemento_nombre}: {det.cantidad_requerida} {det.unidad_medida}
                      </p>
                    </div>
                  ))}
                  {elementosReceta.length > 3 && (
                    <p className="text-xs text-gray-500">
                      +{elementosReceta.length - 3} más
                    </p>
                  )}
                </div>
              </div>
